...
class JudgehostController extends AbstractFOSRestController
{
    ...
    private function serializeJudgeTasks(array $judgeTasks, Judgehost $judgehost): array
    {
        ...
        // NEW: attach problem info to each JudgeTask so judgehost scripts can select harness
        /** @var Submission|null $submission */
        $submission = $this->em->getRepository(Submission::class)
            ->findOneBy(['submitid' => $submit_id]);

        if ($submission !== null) {
            $problem = $submission->getProblem();
            $probid = $problem->getProbid();

            // Prefer contest shortname (A/B/C...) if present, else problem shortname
            $shortname = null;
            try {
                $shortname = $submission->getContestProblem()->getShortname();
            } catch (\Throwable $e) {
                $shortname = $problem->getShortname();
            }

            foreach ($judgeTasks as $jt) {
                $jt->setProblemId($probid);
                $jt->setProblemShortname($shortname);
            }
        }
